...
def print_operation_table(operation, num_rows, num_columns):
    for x in range(1, num_rows + 1):
        ...
        for y in range(1, num_columns + 1):
            my_list.append(str(operation(x, y)))
        print('   '.join(my_list))


my_rows = int(input('Enter count rows: '))
my_columns = int(input('Enter count columns: '))

print_operation_table(lambda x, y: x + y, my_rows, my_columns)
